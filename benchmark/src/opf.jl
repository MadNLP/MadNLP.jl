...
function opf_model(case, backend)
    m, ~ = opf_model("pglib_opf_case6495_rte.m"; backend = MadNLPGPU.CUDABackend())
    return m
end

function benchmark_opf(modeloptions, backend, solveroptions)
    return run_benchmark(opf_model, modeloptions, OPF_CASES, backend, solveroptions)
end
