...
@kwdef mutable struct PardisoOptions <: AbstractOptions
    ...
    pardiso_algorithm::LinearFactorization = BUNCHKAUFMAN
end
...
function PardisoSolver(
    ...
    mtype = if opt.pardiso_algorithm ∈ [BUNCHKAUFMAN, LDL]
    ...
    if opt.pardiso_algorithm == LDL
        iparm[21] = 0 # LDL pivoting
    elseif opt.pardiso_algorithm == BUNCHKAUFMAN
...
function inertia(M::PardisoSolver)
    ...
    if M.err.x < 0
        ...
        if M.opt.pardiso_algorithm == CHOLESKY
            return (n, 0, 0)
        else
            pos = M.iparm[22]
            neg = M.iparm[23]
