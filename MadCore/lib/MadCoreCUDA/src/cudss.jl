MadCore.@kwdef mutable struct CudssSolverOptions <: MadCore.AbstractOptions
    # Use LDLᵀ by default in CUDSS as Cholesky can lead to undefined behavior.
    cudss_algorithm::MadCore.LinearFactorization = MadCore.LDL
...
mutable struct CUDSSSolver{T, V} <: MadCore.AbstractLinearSolver{T}
...
function CUDSSSolver(
    ...
    (opt.cudss_algorithm == MadCore.LU) && error(logger, "The sparse LU of cuDSS is not supported.")
    (opt.cudss_algorithm == MadCore.CHOLESKY) && (structure = "SPD")
    (opt.cudss_algorithm == MadCore.LDL) && (structure = "S")
...
function MadCore.factorize!(M::CUDSSSolver)
...
function MadCore.solve_linear_system!(M::CUDSSSolver{T, V}, xb::V) where {T, V}
    if M.opt.cudss_ir > 0
        copyto!(M.buffer, xb)
...
MadCore.input_type(::Type{CUDSSSolver}) = :csc
MadCore.default_options(::Type{CUDSSSolver}) = CudssSolverOptions()
MadCore.is_inertia(M::CUDSSSolver) = (M.inner.matrix.nbatch == 1)  # Uncomment if MadCore.LU is supported -- (M.opt.cudss_algorithm ∈ (MadCore.CHOLESKY, MadCore.LDL))
function MadCore.inertia(M::CUDSSSolver)
    ...
    return if M.opt.cudss_algorithm == MadCore.CHOLESKY
        if info == 0
            return (n, 0, 0)
        else
            return (n - 2, 1, 1) # if factorization fails, return a dummy inertia
        end
    elseif M.opt.cudss_algorithm == MadCore.LDL
...
MadCore.improve!(M::CUDSSSolver) = false
MadCore.is_supported(::Type{CUDSSSolver}, ::Type{Float32}) = true
MadCore.is_supported(::Type{CUDSSSolver}, ::Type{Float64}) = true
MadCore.introduce(M::CUDSSSolver) = "cuDSS v$(CUDSS.version())"
MadCore.is_async(M::CUDSSSolver) = M.opt.cudss_asynchronous
