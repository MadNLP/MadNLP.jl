for (potrf, potrf_buffer, potrs, nbytes, T) in
    ...
    @eval begin
        function MadCore.setup_cholesky!(M::LapackCUDASolver{$T})
            if M.legacy
        ...
        function MadCore.factorize_cholesky!(M::LapackCUDASolver{$T})
            if M.legacy
                cuSOLVER.$potrf(
                    dense_handle(),
                    CUBLAS_FILL_MODE_LOWER,
        ...
        function MadCore.solve_cholesky!(M::LapackCUDASolver{$T}, x::CuVector{$T})
            if M.legacy
                cuSOLVER.$potrs(
                    dense_handle(),
                    CUBLAS_FILL_MODE_LOWER,
...
for (sytrf_buffer, sytrf, nbytes, T) in
    ...
    @eval begin
        function MadCore.setup_bunchkaufman!(M::LapackCUDASolver{$T})
            resize!(M.ipiv, M.n)
            resize!(M.ipiv64, M.n)
        ...
        function MadCore.factorize_bunchkaufman!(M::LapackCUDASolver{$T})
        ...
        function MadCore.solve_bunchkaufman!(M::LapackCUDASolver{$T}, x::CuVector{$T})
...
for (getrf, getrf_buffer, getrs, nbytes, T) in
    ...
    @eval begin
        function MadCore.setup_lu!(M::LapackCUDASolver{$T})
            if M.legacy
                resize!(M.ipiv, M.n)
        ...
        function MadCore.factorize_lu!(M::LapackCUDASolver{$T})
            if M.legacy
                cuSOLVER.$getrf(
                    dense_handle(),
                    Cint(M.n),
        ...
        function MadCore.solve_lu!(M::LapackCUDASolver{$T}, x::CuVector{$T})
            if M.legacy
                cuSOLVER.$getrs(
                    dense_handle(),
                    CUBLAS_OP_N,
...
for (geqrf, geqrf_buffer, ormqr, ormqr_buffer, trsv, nbytes, T) in
    ...
    @eval begin
        function MadCore.setup_qr!(M::LapackCUDASolver{$T})
            resize!(M.tau, M.n)
        ...
        function MadCore.factorize_qr!(M::LapackCUDASolver{$T})
            if M.legacy
                cuSOLVER.$geqrf(
                    dense_handle(),
                    Cint(M.n),
        ...
        function MadCore.solve_qr!(M::LapackCUDASolver{$T}, x::CuVector{$T})
...
for (syevd, syevd_buffer, gemv, nbytes, T) in
    ...
    @eval begin
        function MadCore.setup_evd!(M::LapackCUDASolver{$T})
            resize!(M.tau, M.n)
            resize!(M.Λ, M.n)
            if M.legacy
        ...
        function MadCore.factorize_evd!(M::LapackCUDASolver{$T})
            if M.legacy
                cuSOLVER.$syevd(
                    dense_handle(),
                    CUSOLVER_EIG_MODE_VECTOR,
        ...
        function MadCore.solve_evd!(M::LapackCUDASolver{$T}, x::CuVector{$T})
            cuBLAS.$gemv(
                handle(),
                CUBLAS_OP_T,
                M.n,
