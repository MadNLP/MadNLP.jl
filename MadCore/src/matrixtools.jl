...
function findIJ(S::SparseMatrixCSC{Tv, Ti}) where {Tv, Ti}
    ...
    cnt = 1
    @inbounds for col in 1:size(S, 2), k in getcolptr(S)[col]:(getcolptr(S)[col + 1] - 1)
        I[cnt] = rowvals(S)[k]
        J[cnt] = col
        cnt += 1
    end
...
function diag!(dest::AbstractVector{T}, src::AbstractMatrix{T}) where {T}
    ...
    return @inbounds for i in eachindex(dest)
        dest[i] = src[i, i]
    end
end
...
function transfer!(dest::SparseMatrixCSC, src::SparseMatrixCOO, map::Vector{Int})
    return _transfer!(dest.nzval, src.V, map)
end
...
function get_cscsy_view(csc::SparseMatrixCSC{Tv, Ti}, Ix; inds = collect(1:nnz(csc))) where {Tv, Ti <: Integer}
    ...
    return SparseMatrixCSC{Tv, Ti}(
            ...
            cscindsub.rowval, Vector{Tv}(undef, nnz(cscindsub))
        ), view(csc.nzval, cscindsub.nzval)
...
function force_lower_triangular!(I, J)
    return @simd for i in 1:length(I)
        @inbounds if J[i] > I[i]
            tmp = J[i]
            J[i] = I[i]
            I[i] = tmp
