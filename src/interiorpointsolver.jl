...
function Solver(nlp::NonlinearProgram;
    ...
    dx= view(d,1:n)
    dl= view(d,n+1:n+m)
    dzl= Vector{Float64}(undef,nlb)
    dzu= Vector{Float64}(undef,nub)
    ...
    return Solver{typeof(kkt)}(nlp,kkt,opt,cnt,logger,
                  ...
                  jacl,
                  d,dx,dl,dzl,dzu,dx_lr,dx_ur,p,px,pl,
                  ...
                  x_lr,x_ur,xl_r,xu_r,zl_r,zu_r,x_trial_lr,x_trial_ur,
...
function second_order_correction(ips::AbstractInteriorPointSolver,alpha_max::Float64,theta::Float64,varphi::Float64,
    ...
    @debug(ips.logger,"Second-order correction started.")

    apply_step!(ips._w1l,ips.c_trial,ips.c,alpha_max)
    ...
    for p=1:ips.opt.max_soc
        ...
        apply_step!(ips.x_trial,ips.x,ips._w1x,alpha_soc)
        eval_cons_wrapper!(ips, ips.c_trial,ips.x_trial)
        ips.obj_val_trial = eval_f_wrapper(ips, ips.x_trial)
...
function finish_aug_solve!(ips::Solver, kkt::AbstractKKTSystem, mu)
    ips.dzl.= (ips.mu.-ips.zl_r.*ips.dx_lr)./(ips.x_lr.-ips.xl_r).-ips.zl_r
    ips.dzu.= (ips.mu.+ips.zu_r.*ips.dx_ur)./(ips.xu_r.-ips.x_ur).-ips.zu_r
end
...
function fixed_variable_treatment_z!(zl,zu,f,jacl,ind_fixed)
    @simd for i in ind_fixed
        @inbounds z = f[i]+jacl[i]
        @inbounds z >=0 ? (zl[i] = z; zu[i] = 0.) : (zl[i] = 0.; zu[i] = -z)
    end
end
