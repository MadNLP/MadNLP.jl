...
function treat_fixed_variable!(kkt::AbstractKKTSystem{T, MT}) where {T, MT<:Matrix{T}}
    length(kkt.ind_fixed) == 0 && return
    aug = kkt.aug_com
    @simd for i in kkt.ind_fixed
        @inbounds aug[i, :] .= 0.0
        @inbounds aug[:, i] .= 0.0
        @inbounds aug[i, i]  = 1.0
    end
end
...
function set_jacobian_scaling!(kkt::AbstractSparseKKTSystem{T, MT}, constraint_scaling::AbstractVector) where {T, MT}
    ...
    @simd for i in 1:nnzJ
        @inbounds index = kkt.jac_raw.I[i]
        @inbounds kkt.jacobian_scaling[i] = constraint_scaling[index]
    end
end

function compress_jacobian!(kkt::AbstractSparseKKTSystem{T, MT}) where {T, MT<:SparseMatrixCSC{T, Int32}}
    ns = length(kkt.ind_ineq)
    _compress_jacobian_sparse!(ns,kkt.jac,kkt.jacobian_scaling)
    transfer!(kkt.jac_com, kkt.jac_raw, kkt.jac_csc_map)    
end

function _compress_jacobian_sparse!(ns,jac,jacobian_scaling)
    jac[end-ns+1:end] .= -1.0
    jac .*= jacobian_scaling # scaling
end
