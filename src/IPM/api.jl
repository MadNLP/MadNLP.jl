...
set_sc!(solver::AbstractMadNLPSolver{T}, rhs::T) where {T} = solver.sc = rhs
set_sd!(solver::AbstractMadNLPSolver{T}, rhs::T) where {T} = solver.sd = rhs
...
_inf_compl(solver::AbstractMadNLPSolver{T}; mu=_mu(solver)) where {T} = get_inf_compl(_x_lr(solver),_xl_r(solver),_zl_r(solver),_xu_r(solver),_x_ur(solver),_zu_r(solver), mu, _sc(solver))
