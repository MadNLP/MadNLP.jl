...
function MadNLP.diag!(dest::ROCVector{T}, src::ROCMatrix{T}) where {T}
    ...
    backend = ROCBackend()
    _copy_diag_kernel!(backend)(dest, src, ndrange = length(dest))
    synchronize(backend)
    return
end
...
function MadNLP.diag_add!(dest::ROCMatrix, src1::ROCVector, src2::ROCVector)
    backend = ROCBackend()
    _add_diagonal_kernel!(backend)(dest, src1, src2, ndrange = size(dest, 1))
    synchronize(backend)
    return
end
...
function MadNLP._set_diag!(A::ROCMatrix, inds, a)
    if !isempty(inds)
        backend = ROCBackend()
        _set_diag_kernel!(backend)(A, inds, a; ndrange = length(inds))
        synchronize(backend)
    end
    return
end
...
function MadNLP._build_dense_kkt_system!(
    ...
    ndrange = (n + m + ns, n)
    backend = ROCBackend()
    _build_dense_kkt_system_kernel!(backend)(
        dest,
        hess,
        jac,
        pr_diag,
...
function MadNLP._build_ineq_jac!(
    ...
    ndrange = (m_ineq, n)
    backend = ROCBackend()
    _build_jacobian_condensed_kernel!(backend)(
        dest,
        jac,
        diag_buffer,
        ind_ineq_gpu,
...
function MadNLP._build_condensed_kkt_system!(
    ...
    ndrange = (n + m_eq, n)
    backend = ROCBackend()
    _build_condensed_kkt_system_kernel!(backend)(
        dest,
        hess,
        jac,
        pr_diag,
