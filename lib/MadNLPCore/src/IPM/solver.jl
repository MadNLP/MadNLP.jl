...
function initialize!(solver::AbstractMadNLPSolver{T}) where {T}
    ...
    set_theta_max!(solver, T(1.0e4) * max(one(T), theta))
    set_theta_min!(solver, T(1.0e-4) * max(one(T), theta))
...
function initialize_dual(solver::AbstractMadNLPSolver{T}, ::Type{DualInitializeSetZero}) where {T}
    return fill!(get_y(solver), zero(T))
end
function initialize_dual(solver::AbstractMadNLPSolver{T}, ::Type{DualInitializeLeastSquares}) where {T}
    ...
    return if !is_solved || (norm(dual(get_d(solver)), T(Inf)) > get_opt(solver).constr_mult_init_max)
...
function reinitialize!(solver::AbstractMadNLPSolver)
    ...
    theta = get_theta(get_c(solver))
    set_theta_max!(solver, 1.0e4 * max(1, theta))
    set_theta_min!(solver, 1.0e-4 * max(1, theta))
    set_mu!(solver, get_opt(solver).barrier.mu_init)
    ...
    while true
        ...
        get_inf_total(solver) <= get_opt(solver).acceptable_tol ?
            (
                ...
                get_cnt(solver).acceptable_cnt += 1 : return SOLVED_TO_ACCEPTABLE_LEVEL
            ) : (get_cnt(solver).acceptable_cnt = 0)
...
function second_order_correction(
        solver::AbstractMadNLPSolver, alpha_max, theta, varphi,
    ...
    for p in 1:get_opt(solver).max_soc
...
function inertia_correction!(
    ...
    while !solve_status
        ...
        if n_trial == 0
            set_del_w!(
                solver, get_del_w_last(solver) == zero(T) ? get_opt(solver).first_hessian_perturbation :
...
function inertia_correction!(
    ...
    while !curv_test(t, n, g, get_kkt(solver), wx, get_opt(solver).inertia_free_tol) || !solve_status
        ...
        if n_trial == 0
            set_del_w!(
                solver, get_del_w_last(solver) == 0.0 ? get_opt(solver).first_hessian_perturbation :
                    max(get_opt(solver).min_hessian_perturbation, get_opt(solver).perturb_dec_fact * get_del_w_last(solver))
            )
        else
            set_del_w!(solver, get_del_w(solver) * (get_del_w_last(solver) == 0.0 ? get_opt(solver).perturb_inc_fact_first : get_opt(solver).perturb_inc_fact))
            if get_del_w(solver) > get_opt(solver).max_hessian_perturbation
                get_cnt(solver).k += 1
                @debug(get_logger(solver), "Primal regularization is too big. Switching to restoration phase.")
                return false
...
function curv_test(t, n, g, kkt, wx, inertia_free_tol)
    ...
    return dot(wx, t) + max(dot(wx, n) - dot(g, n), 0) - inertia_free_tol * dot(t, t) >= 0
end
