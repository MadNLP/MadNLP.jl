...
function filter_line_search!(solver::AbstractMadNLPSolver{T}) where {T}
    ...
    alpha_min = get_alpha_min(
        theta, varphi_d, get_theta_min(solver), get_opt(solver).gamma_theta, get_opt(solver).gamma_phi,
    ...
    switching_condition = is_switching(varphi_d, get_alpha(solver), get_opt(solver).s_phi, get_opt(solver).delta, 2.0, get_opt(solver).s_theta)
...
function filter_line_search_RR!(solver::AbstractMadNLPSolver{T}) where {T}
    ...
    alpha_min = get_alpha_min(
        theta_R, varphi_d_R, get_theta_min(solver), get_opt(solver).gamma_theta, get_opt(solver).gamma_phi,
        get_opt(solver).alpha_min_frac, get_opt(solver).delta, get_opt(solver).s_theta, get_opt(solver).s_phi
    )
